...
<mssqlcrt:limit-max-decimal-precision-implicit-cast>26</mssqlcrt:limit-max-decimal-precision-implicit-cast>
...
<mssqlcrt:type>Microsoft.DataWarehouse.Design.OracleSchema, Microsoft.DataWarehouse.AS</mssqlcrt:type>
<mssqlcrt:query-designer>
<mssqlcrt:type>Microsoft.DataWarehouse.Controls.OracleQueryDesigner, Microsoft.DataWarehouse.AS</mssqlcrt:type>
</mssqlcrt:query-designer>
</mssqlcrt:schema-class>
<mssqlcrt:schema-class>
...
<xsl:template match="OpUNICHAR">NCHR(<xsl:apply-templates select="./*[1]"/>)</xsl:template>
...
<xsl:otherwise>
CAST(<xsl:value-of select="$effectiveValue"/> AS BINARY_DOUBLE)
</xsl:otherwise>
</xsl:choose>
</xsl:otherwise>
</xsl:choose>
...
(SELECT LOG(Op2, Op1) FROM (SELECT <xsl:value-of select="$Op1"/> AS Op1, <xsl:value-of select="$Op2"/> AS Op2 FROM DUAL) AuxTable
</xsl:template>
